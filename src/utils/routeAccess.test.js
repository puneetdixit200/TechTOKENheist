...
test('player startup keeps the Infinite Void countdown over the TechToken loading video', () => {
  const app = readProjectFile('src/App.jsx')
  ...
  assert.match(matchStartOverlay, /\*\*\/\*\.\{mp4,webm,mov,m4v\}/)
  assert.match(matchStartOverlay, /\.\.\/\.\.\/\*\.\{mp4,webm,mov,m4v\}/)
  assert.match(matchStartOverlay, /loading-video-bg/)
  assert.match(matchStartOverlay, /LIMITLESS COUNTDOWN/)
  assert.match(matchStartOverlay, /DOMAIN EXPANSION/)
  assert.match(matchStartOverlay, /ANANT SHUNYATA/)
  assert.match(matchStartOverlay, /INFINITE VOID/)
  assert.match(matchStartOverlay, /SECONDS UNTIL BREACH/)
  assert.doesNotMatch(matchStartOverlay, /fetch\(/)
  assert.doesNotMatch(matchStartOverlay, /<div className="background-container" \/>/)
  assert.match(matchStartOverlay, /<div className="background-container" aria-hidden="true" \/>/)
  assert.match(matchStartCss, /loading-video-bg/)
  assert.match(matchStartCss, /void_bg\.png/)
  assert.match(matchStartCss, /infinite-void-ring/)
  assert.match(matchStartCss, /void-eye-mark/)
