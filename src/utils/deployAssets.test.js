...
test('production build prunes unused copied public assets', () => {
  ...
  const vercelBuildVerifier = readProjectFile('scripts/verify-vercel-build-env.js')
  ...
  assert.match(vercelBuildVerifier, /id,is_connected,last_seen_at/)
  assert.match(vercelBuildVerifier, /countdown_started_at,countdown_duration_ms/)
  assert.match(vercelBuildVerifier, /parseEnvFile/)
  assert.match(vercelBuildVerifier, /\.env\.local/)
  assert.match(vercelBuildVerifier, /\.env\.production\.local/)
  assert.match(vercelBuildVerifier, /\.\.\.fileValues, \.\.\.process\.env/)
  ...
  assert.match(configuredBrowserVerifier, /\/functions\/v1\/game-actions/)
  assert.match(configuredBrowserVerifier, /\{\s*success:\s*true,\s*data:\s*\{\}\s*\}/)
  ...
  assert.match(configuredBrowserVerifier, /\['\.mp4', 'video\/mp4'\]/)
  assert.match(configuredBrowserVerifier, /\['\.webm', 'video\/webm'\]/)
...
test('deployment package ignores local agent caches and one-off artifacts', () => {
  ...
  for (const ignoreFile of [vercelIgnore, npmIgnore]) {
    ...
    assert.doesNotMatch(ignoreFile, /Create_a_hyper_cinematic_D_im\.mp4/)
  ...
  assert.doesNotMatch(gitIgnore, /Create_a_hyper_cinematic_D_im\.mp4/)
