...
test('admin confirms wheel-selected domain before creating a match', () => {
  ...
  assert.match(adminScreen, /pairTeamA\?\.tokens \?\? 0\}\s*TKN<\/span>[\s\S]*\{pair\.teamAName\}/)
  assert.match(adminScreen, /\{pair\.teamBName\}[\s\S]*pairTeamB\?\.tokens \?\? 0\}\s*TKN<\/span>/)
  assert.doesNotMatch(adminScreen, /Current Tokens/)
  assert.doesNotMatch(adminScreen, /Stake Scan/)
...
test('finale screen avoids emoji glyph alignment and exposes music controls before victory', () => {
  ...
  const finaleAudioBlock = finaleCss.slice(
    ...
    finaleCss.indexOf('.finale-audio-title {'),
  )
  assert.match(finaleAudioBlock, /top:\s*88px/)
  assert.doesNotMatch(finaleAudioBlock, /bottom:\s*22px/)
  assert.match(finaleCss, /\.showdown-header-panel \{[\s\S]*min-height:\s*76px/)
